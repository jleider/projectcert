...
Snapshot: cct-rubric-2017.pdf (in same directory; 659.6 KB)
...
- Coding `linguistic: false` is a coding correction against the
  ...
  professional-standards document, so we follow what is actually in
  the CCT.
- See changes-from-baseline.md for further discussion; this is the
  most ambiguous of the four flags.
...
- This is an explicit, named reference to ELs in the foundational
  attribute (Indicator 1a) of the rubric. The 2014 baseline coded
  this `false`; the 2017 rubric makes the EL reference explicit.

## Bottom line

Flags after this read:
- diverse: true (unchanged from baseline)
