...
- https://www.eslteacheredu.org/connecticut/  (third-party explainer
  of the CSDE Initial TESOL Teaching Certificate, useful for the
  standalone-vs-cross distinction since the certification.ct.gov
  certification-area page redirected at last check)
...
ESOL beyond what the underlying certificate requires; the test
requirement applies to the standalone Initial TESOL cert.
...
TESOL credential rolls up under `eld`. Because `standalone: true`
exists and is the broader pathway, schema fields are coded on the
strength of the standalone pathway:
...
Baseline coded:
- program: null  → now true (Initial TESOL cert is a state-approved
  preparation program)
...
The baseline appears to have left fields null because the distinction
between cross-endorsement (coursework only) and Initial TESOL cert
(full program with student teaching and Praxis) was not fully resolved
in 2019. The current CSDE pages document both pathways clearly, so
where a current source clearly establishes true/false we replace null.
