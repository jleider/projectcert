...
export function breakdownFor(states: State[], type: "bilingual" | "eld"): CredentialBreakdown {
  ...
  const addOnOnly = offered.filter((s) => s.credentials[type].addOn && !s.credentials[type].standalone);
  const both = offered.filter((s) => s.credentials[type].standalone && s.credentials[type].addOn);
