...
describe("/api/verifications", () => {
  it("upserts, lists, overwrites (single-check), and deletes", async () => {
    const post = await verPost(
      ctx({
        method: "POST",
        body: { usps: "CA", datapoint_id: "elPercent", content_hash: "h1" },
      }),
    );
    ...
    const del = await verDel(
      ctx({
        method: "DELETE",
        body: { usps: "CA", datapoint_id: "elPercent" },
      }),
    );
    ...
    expect(
      (
        await verPost(
          ctx({
            method: "POST",
            body: { usps: "CA", datapoint_id: "bogus", content_hash: "h" },
          }),
        )
      ).status,
    ).toBe(400);
    ...
    const post = await sugPost(
      ctx({
        ...
        body: {
          ...
          body: "Source 2 link is outdated.",
        },
      }),
    );
    expect(post.status).toBe(200);
    ...
    expect(
      (
        await sugPost(
          ctx({
            method: "POST",
            body: { usps: "TX", datapoint_id: "sources", body: "   " },
          }),
        )
      ).status,
    ).toBe(400);
...
describe("/api/overview", () => {
  it("aggregates verified and broken counts per state", async () => {
    await verPost(
      ctx({
        method: "POST",
        body: { usps: "CA", datapoint_id: "elPercent", content_hash: "h" },
      }),
    );
    await verPost(
      ctx({
        method: "POST",
        body: { usps: "CA", datapoint_id: "sources", content_hash: "h" },
      }),
    );
    db.prepare(
...
describe("/api/link-reviews", () => {
  ...
  it("accepts and reverts a URL", async () => {
    ...
    const revert = await lrPost(
      ctx({
        method: "POST",
        body: { url: "https://azed.gov", decision: "pending" },
      }),
    );
    ...
    expect(
      (
        await lrPost(
          ctx({
            method: "POST",
            body: { url: "https://nope", decision: "accepted" },
          }),
        )
      ).status,
    ).toBe(404);
    expect(
      (
        await lrPost(
          ctx({
            method: "POST",
            body: { url: "https://azed.gov", decision: "maybe" },
          }),
        )
      ).status,
    ).toBe(400);
